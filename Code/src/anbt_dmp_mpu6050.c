...
u8 AnBT_DMP_MPU6050_DEV_CFG(void)
{
    ...
    if (AnBT_DMP_I2C_Read(st.hw->addr, st.reg->accel_offs, 6, anbt_dmp_data))
			return 5;	//etootle:Check product revision  		������Ʒ����
    ...
    if (mpu_set_bypass(0)) return 0x15;					//��ͬ��
...
u8 AnBT_DMP_MPU6050_Init(void)
{
	...
	printf("P-1,Finish DMP device init."); 		//���ɳ�ʼ��
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-2,Finish DMP set sensor.");		//���ɴ���ȥ�趨
	else printf("P-2,DMP set sensor error.");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-3,Finish DMP configure fifo.");		//����fifo����
	else printf("P-3,DMP configure fifo error.");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))printf("P-4,Finish DMP set sample rate.");			//����DMP����������
	else printf("P-4,DMP set sample_rate error.");
	//
	if(!dmp_load_motion_driver_firmware())	printf("P-5,Finish DMP load motion driver firmware.");				//����DMP����
	else printf("P-5,DMP load motion driver firmware error.");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("P-6,Finish DMP set orientation.");		//��ʼ�����趨ƫ��
	else printf("P-6,DMP set orientation error.");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		printf("P-7,Finish DMP enable feature.");		//DMPʹ��
	else printf("P-7,DMP enable feature error.");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) printf("P-8,Finish DMP set fifo rate.");		//DMP  fifo�ٶ�
	else printf("P-8,DMP set fifo rate error.");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) printf("P-9,Finish DMP set dmp state.");				//DMP�ɹ�����
	else printf("P-9,DMP set dmp state error.");					//Dmp����ʧ��
	//
	return 0;
}

// void AnBT_DMP_MPU6050_SEND_DATA_FUN(void)	//dmp��ȡ����
// {
// 	dmp_read_fifo(gyro, accel, anbt_mpu6050_quat_data, &sensor_timestamp, &sensors, &more);
// //	ANBT_SEND_DMP_DATA(anbt_mpu6050_quat_data);		//����DMP����
// }
